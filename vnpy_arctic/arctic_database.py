...
class ArcticMongoDatabase(BaseDatabase):
    ...
    def load_bar_data(
        ...
        if bar_symbol in self.library.list_symbols():
            ...
            for date, symbol, exchange_d, interval, volume, open_interest, open_price, high_price, low_price, close_price\
                in zip(df["date"], df["symbol"], df["exchange"], df["interval"], df["volume"], df["open_interest"],
                       df["open_price"], df["high_price"], df["low_price"], df["close_price"]):
    ...
    def load_tick_data(
        ...
        if tick_symbol in self.library.list_symbols():
            ...
            ticks: List[TickData] = []
            for symbol, exchange_t, date, name, volume,\
                ...
                in zip(df["symbol"], df["exchange"], df["date"], df["name"], df["volume"],
